...
void ReadFile(const char* filename, map<string, Color>& colors)
{
    ...
    if (File.is_open()) {
        ...
        while (getline(File, lineFromFile)) {
            ...
            getline(stream, tempColorName, ' ');
            getline(stream, tempColorInt, ' ');
            colorInt = stoi(tempColorInt);
            colorName = tempColorName.c_str();


            Color newColor;
            newColor.SetName(colorName);
            newColor.SetValue(colorInt);
            colors.emplace(colorName, newColor);
...
void SearchForColor(map<string, Color>& colors)
{
    ...
    string searchColor;
    cin >> searchColor;
    auto search = colors.find(searchColor);
    if (search != colors.end()) {
        PrintColor(search->second);
    } else {
